...
class ParcelGen:
    ...
    CLASS_STR = "public class %s implements %s {"
    ...
    def print_gen(self, props, class_name, package, imports, transient):
        ...
        for typ, member in self.member_map():
            if member in transient:
                typ = "transient " + typ
            self.printtab("private %s %s;" % (typ, self.memberize(member)))
        self.output("")

        # Parameterized Constructor
        constructor = "public %s(" % class_name
        params = []
        for typ, member in self.member_map():
        ...
        self.uptab()
        for typ, member in self.member_map():
            self.printtab("this.%s = %s;" % (self.memberize(member), member))
        self.tablevel -= 1
        self.printtab("}\n")

        # Empty constructor for Parcelable
        self.printtab("public %s(Parcel p) {" % class_name)
        self.uptab()
        self.printtab("readFromParcel(p);")
        self.downtab()
        self.printtab("}\n")
